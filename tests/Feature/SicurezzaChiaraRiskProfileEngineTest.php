...
test('company and worker risk profiles are deduced from mapped domain sources', function () {
    ...
    $this->actingAs($user)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ...
            ->where('workspaceBridge.operationalQueue.0.key', 'reviews')
            ->where('workspaceBridge.operationalQueue.0.laneLabel', 'Corsia review')
            ->where('workspaceBridge.operationalQueue.0.tone', 'secondary')
