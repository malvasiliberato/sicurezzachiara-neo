...
test('company dvr initial report is derived from real company risk and measure data', function () {
    ...
    $this->actingAs($user)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ...
            ->where('lightScope.title', 'Cosa mostra gia\' il DVR light')
            ->where('lightScope.reads.0.label', 'Contesto e sorgenti gia\' censite')
            ->where('lightScope.notIncluded.0.label', 'Non sostituisce un DVR documentale maturo')
            ...
            ->has('lightScope.reads', 4)
            ->has('lightScope.notIncluded', 3)
            ->has('lightScope.nextSteps')
            ->etc()
        )
