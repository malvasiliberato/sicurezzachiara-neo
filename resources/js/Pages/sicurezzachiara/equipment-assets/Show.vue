...
const suggestedRisks = computed(() =>
  (props.asset.equipment_type?.risk_source_links ?? []).map((link) => ({
    id: link.id,
...
<template>
  <Layout>
    ...
    <PageHeader title="Dettaglio macchinario" pageTitle="SicurezzaChiara" />
      ...
      <BCardBody class="p-4">
        <div class="d-flex align-items-start justify-content-between gap-3 flex-wrap">
          <div>
            ...
            <p class="text-muted mb-3">{{ asset.notes || "Nessuna nota operativa presente." }}</p>
            <div class="hstack gap-2 flex-wrap">
              <span class="badge bg-light text-body">{{ tenant.name }}</span>
              <span class="badge bg-soft-info text-info">{{ asset.company?.name || "Azienda non disponibile" }}</span>
              <span class="badge bg-soft-secondary text-secondary">{{ asset.site?.name || "Sede non assegnata" }}</span>
          ...
          <div class="hstack gap-2 flex-wrap">
            <Link v-if="$page.props.tenantContext?.permissions?.can_manage_data" :href="route('equipment-assets.edit', asset.id)" class="btn btn-primary">Modifica macchinario</Link>
            <Link :href="route('equipment-assets.index', { company_id: asset.company?.id })" class="btn btn-soft-secondary">Torna ai macchinari azienda</Link>
          </div>
        </div>
      </BCardBody>
    </BCard>
        ...
        <BTabs nav-class="nav-success mb-4" pills>
            ...
            <BRow class="g-3">
              <BCol md="6" xl="3">
                <div class="border rounded-3 p-3 h-100">
                  <span class="text-muted d-block fs-13">Tipologia macchinario</span>
                  <div class="fw-semibold mt-1">{{ asset.equipment_type?.name || "Non disponibile" }}</div>
                </div>
              </BCol>
              <BCol md="6" xl="3">
                <div class="border rounded-3 p-3 h-100">
                  <span class="text-muted d-block fs-13">Sede</span>
                  <div class="fw-semibold mt-1">{{ asset.site?.name || "Non assegnata" }}</div>
                </div>
              </BCol>
              <BCol md="6" xl="3">
                <div class="border rounded-3 p-3 h-100">
                  <span class="text-muted d-block fs-13">Costruttore / modello</span>
                  <div class="fw-semibold mt-1">{{ asset.manufacturer || "N.D." }} / {{ asset.model || "N.D." }}</div>
                </div>
              </BCol>
              <BCol md="6" xl="3">
                <div class="border rounded-3 p-3 h-100">
                  <span class="text-muted d-block fs-13">Codice bene</span>
                  <div class="fw-semibold mt-1">{{ asset.asset_code || "Non indicato" }}</div>
                </div>
              </BCol>
            </BRow>
          </BTab>
          ...
          <BTab title="Rischi suggeriti">
            <div v-if="suggestedRisks.length === 0" class="text-center py-4">
              ...
              <p class="text-muted mb-0">La tipologia del macchinario non ha ancora collegamenti espliciti nel catalogo rischi.</p>
            </div>
          ...
          <BTab title="Presidi e scadenze">
            ...
            <div v-if="governanceContext.previewMeasures.length === 0" class="text-center py-4">
              ...
              <p class="text-muted mb-0">Per i rischi suggeriti da questo macchinario non emergono ancora misure o scadenze nel contesto aziendale collegato.</p>
            </div>
            <div v-else class="table-responsive">
              <table class="table align-middle table-nowrap mb-0">
                <thead class="table-light">
          ...
          <BTab :title="`Lavoratori collegati (${asset.worker_exposures.length})`">
            <div v-if="asset.worker_exposures.length === 0" class="text-center py-4">
              <h5 class="mb-2">Nessuna associazione presente</h5>
              <p class="text-muted mb-0">Il macchinario e' censito ma non e' ancora stato collegato ai lavoratori.</p>
            </div>
            <div v-else class="table-responsive">
              <table class="table align-middle table-nowrap mb-0">
                <thead class="table-light">
                ...
                <tbody>
                  <tr v-for="exposure in asset.worker_exposures" :key="exposure.id">
